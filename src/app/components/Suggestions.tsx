...
import { Bath, BedDouble, Info, Ruler, Star } from 'lucide-react'
import React from 'react'
import { Button } from '@/components/ui/button'
import Image from 'next/image'
import image1 from '../../../public/image1.png'

// Assuming IconText is a local component like this:
...
const hotelData = {
  // ... (rest of your hotelData can stay the same)
  suggestions: Array(6).fill({
    ...
    baths: 2,
    size: 1200,
    // ✨ Using the image from your screenshot
    image: image1,
  }),
}
    ...
    <div className='container mx-auto px-4'>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2'>
        {hotelData.suggestions.map((item, i) => (
          <Card
            key={i}
            className=' m-0 gap-0 h-full p-0 overflow-hidden  border-none group transition-all duration-300 '
          >
            {/* Image section */}
            <div className='relative'>
              <Image src={item.image} alt={item.name} className='w-full h-44 object-cover rounded-2xl' width={500} height={400}  />
            ...
            <CardContent className='p-5'>
              ...
              <p className='text-sm text-gray-500 mt-1 overflow-hidden'>{item.description}</p>
